...
class DiscreteKeyValueBottleneck(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            embed_dim % num_memory_codebooks
        ) == 0, f"embedding dimension {embed_dim} must be divisible by number of codes {num_memory_codebooks}"
    ...
    def forward(self, x, return_intermediates=False, **kwargs):
        ...
        values = repeat(
            self.values, "h n d -> b h n d", b=memory_indices.shape[0]
        )
        memory_indices = repeat(
            memory_indices, "b h n -> b h n d", d=values.shape[-1]
        )
