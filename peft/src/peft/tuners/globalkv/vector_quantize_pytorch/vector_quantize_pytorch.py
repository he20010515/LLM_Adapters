...
class EuclideanCodebook(nn.Module):
    def __init__(
        ...
        self.reset_cluster_size = default(
            reset_cluster_size, threshold_ema_dead_code
        )
    ...
    def update_affine(self, data, embed):
        ...
        data, embed = map(
            lambda t: rearrange(t, "h ... d -> h (...) d"), (data, embed)
        )
...
class VectorQuantize(nn.Module):
    def __init__(
        ...
        codebook_class = (
            EuclideanCodebook if not use_cosine_sim else CosineSimCodebook
        )
    ...
    def forward(self, x, indices=None, mask=None, sample_codebook_temp=None):
        ...
        if self.training:
            ...
            maybe_detach = (
                torch.detach if not self.learnable_codebook else identity
            )
