...
class MaskedPrototype(nn.Module):
    ...
    def __init__(self, parent: Prototype, sparsity: float) -> None:
        ...
        self.trainable_floating_scoreA = nn.Parameter(
            torch.ones_like(
                parent.lora_A_prototype.weight, dtype=torch.float32
            )
        )
        self.trainable_floating_scoreB = nn.Parameter(
            torch.ones_like(
                parent.lora_B_prototype.weight, dtype=torch.float32
            )
        )

    def forward(self, x):
        A = (
            subnet_puring.apply(self.trainable_floating_scoreA, self.sparsity)
            * self.parent.lora_A_prototype.weight
        )
        B = (
            subnet_puring.apply(self.trainable_floating_scoreB, self.sparsity)
            * self.parent.lora_B_prototype.weight
        )
        x = F.linear(x, A)
        x = F.linear(x, B)
        return x
...
class PrototypeLoraConfig(LoraConfig):
    ...
    sparsity: Optional[float] = field(
        default=0.1, metadata={"help": "the sparsity of the binary mask"}
    )
...
class PrototypeLoraLayer:
    ...
    def update_layer(
        ...
        if r > 0:
            self.lora_AB.update(
                nn.ModuleDict(
                    {
                        adapter_name: MaskedPrototype(
                            parent=self.parent, sparsity=self.sparsity
                        )
                    }
                )
            )
    ...
    def reset_lora_parameters(self, adapter_name):
        warnings.warn(
            " reset_lora_parameters NOT Implemented ,current use One's init"
        )
...
class Linear(nn.Linear, PrototypeLoraLayer):
    def __init__(
        ...
        **kwargs,
        ...
        PrototypeLoraLayer.__init__(
            self,
            in_features,
            out_features,
            sparsity=sparsity,
            parent=prototype_pointer,
        )
...
class PrototypeLoraModel(nn.Module):
    ...
    def __init__(self, model, config, adapter_name):
        ...
        self.prototype = Prototype(
            r=config["default"].r,
            in_features=lora_input,
            out_features=lora_output,
        )
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        kwargs = {
            ...
            "init_lora_weights": getattr(
                lora_config, "init_lora_weights", None
            ),
        }
        ...
        new_module = Linear(
            ...
            in_features,
            out_features,
            sparsity=lora_config.sparsity,
            bias=bias,
            prototype_pointer=self.prototype,
            **kwargs,
        )
        return new_module
    ...
    def add_adapter(self, adapter_name, config=None):
        ...
        if (
            ...
            and self.peft_config[adapter_name].bias != "none"
        ):
            raise ValueError(
