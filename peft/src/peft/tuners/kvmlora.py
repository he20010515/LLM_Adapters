...
@dataclass
class KVMLoraConfig(PeftConfig):
    """
    ...
    codebook_nums: int = field(
        default=4, metadata={"help": "The num of codebooks"}
    )
    mem_nums: int = field(
        default=4, metadata={"defaut": "The num of mems per codebook"}
    )
    ...
    **Attributes**:
    ...
    def _find_and_replace(self):
        ...
        kwargs = {
            ...
            "merge_weights": (
                self.peft_config.merge_weights
                or self.peft_config.inference_mode
            )
    ...
    def _set_adapter_layers(self, enabled=True):
        for module in self.model.modules():
            if isinstance(module, KVMLoraLayer):
...
class Linear(nn.Linear, KVMLoraLayer):
    ...
    def __init__(
        ...
        config: Optional[KVMLoraConfig] = None,
        **kwargs,
    ):
        nn.Linear.__init__(self, in_features, out_features, **kwargs)
        KVMLoraLayer.__init__(
            self,
            r=r,
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            if self.r > 0 and self.merged:
                ...
                self.weight.data -= (
                    transpose(
                        matmul_output.to(previous_dtype), self.fan_in_fan_out
                    )
                    * self.scaling
                )
                self.merged = False
...
class MergedLinear(nn.Linear, KVMLoraLayer):
    # Lora implemented in a dense layer
    def __init__(
        self,
        in_features: int,
        ...
        **kwargs,
        ...
        if r > 0 and any(enable_lora):
            self.lora_A = nn.Linear(
                in_features, r * sum(enable_lora), bias=False
            )
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.r > 0 and any(self.enable_lora):
                ...
                self.weight.data += transpose(
                    self.zero_pad(delta_w * self.scaling),
                    not self.fan_in_fan_out,
                )
            self.merged = True
        elif self.merge_weights and self.merged:
            # Make sure that the weights are not merged
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                ...
                after_B = self.lora_B(after_A.transpose(-2, -1)).transpose(
                    -2, -1
                )
...
if is_bnb_available():
    ...
    class Linear8bitLt(bnb.nn.Linear8bitLt, KVMLoraLayer):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features,
        ...
        def forward(self, x: torch.Tensor):
            ...
            if self.disable_adapters:
                ...
                if not torch.is_autocast_enabled():
                    ...
                    output = (
                        self.lora_B(self.lora_A(self.lora_dropout(x)))
                        * self.scaling
                    )
                    result += output
            return result

    class MergedLinear8bitLt(bnb.nn.Linear8bitLt, KVMLoraLayer):
        # Lora implemented in a dense layer
        def __init__(
            self,
            in_features: int,
        ...
        def forward(self, x: torch.Tensor):
            ...
            if self.disable_adapters:
                ...
                if not torch.is_autocast_enabled():
                    ...
                    output = (
                        self.zero_pad(after_B).to(expected_dtype)
                        * self.scaling
                    )
                    result += output
                else:
