...
class PeftModel(PushToHubMixin, torch.nn.Module):
    ...
    **Attributes**:
    ...
    def __init__(self, model, peft_config: PeftConfig):
        ...
        if isinstance(self.peft_config, PromptLearningConfig):
            ...
            if self.peft_config.peft_type == PeftType.LORA:
                ...
                self.base_model = PrototypeLoraModel(
                    model,
                    config={"default": peft_config},
    ...
    def save_pretrained(self, save_directory, **kwargs):
        ...
        torch.save(
            output_state_dict, os.path.join(save_directory, WEIGHTS_NAME)
        )
    ...
    def _setup_prompt_encoder(self):
        ...
        if self.peft_config.peft_type == PeftType.PROMPT_TUNING:
            prompt_encoder = PromptEmbedding(
                self.peft_config, self.word_embeddings
            )
    ...
    def get_prompt_embedding_to_save(self):
        ...
        """
        prompt_tokens = (
            self.prompt_tokens.unsqueeze(0).expand(1, -1).to(self.device)
        )
    ...
    def get_prompt(self, batch_size):
        ...
        """
        prompt_tokens = (
            self.prompt_tokens.unsqueeze(0)
            .expand(batch_size, -1)
            .to(self.device)
        )
        if self.peft_config.peft_type == PeftType.PREFIX_TUNING:
            ...
            if self.peft_config.inference_mode:
                prompts = self.prompt_encoder.embedding.weight.repeat(
                    batch_size, 1, 1
                )
            else:
